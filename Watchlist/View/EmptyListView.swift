...
struct EmptyListView: View {
    var body: some View {
        ContentUnavailableView {
            ...
            GroupBox {
                TabView {
                    PageTabView.init(icon: "1.circle", description: "Add some movies to the watchlist if you plan to watch later")
                        .padding(.bottom, 36)
                    PageTabView.init(icon: "2.circle", description: "Later on you can also choose a random film to watch")
                        .padding(.bottom, 36)
                }
